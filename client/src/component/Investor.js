...
const Investor = () => {
  ...
  const fetchMyBids = async () => {
    ...
    setError('');
    try {
        const response = await axios.post('/startup/getbid', { userId: "680332e67992d7dbb44933c0" });
  ...
  const handlePlaceBid = async () => {
    ...
    try {
      ...
      const payload = {
        userId: "680332e67992d7dbb44933c0", 
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(350px, 1fr))', gap: '20px' }}>
            {myBids.map(startup => {
              ...
              const userBid = startup.biddedBy.find(bid => {
                ...
                return bid.userId.toString() === newUser.userid;
              });
              return (
                <div 
                  key={startup._id} 
